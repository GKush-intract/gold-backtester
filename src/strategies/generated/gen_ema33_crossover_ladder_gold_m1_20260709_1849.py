...
class EMA33CrossoverLadderGoldM1(Strategy):
    ...
    Long-only momentum entry on XAUUSD m1 bars with a linear TP ladder.
    """
    ...
    params = {
        ...
        "ladder_start_r": ("float", 2.0, 0.5, 10.0, "First take-profit level expressed as a multiple of R (subsequent levels increase by 2R each time)."),
    ...
    def on_bar(self, ctx):
        """Handle execution per bar, ladder management, and entries."""
        p = self.p
        ...
        if ctx.position is not None:
            if not self._in_trade:
                ...
                if self._next_tp > 0 and current_high >= self._next_tp:
                    ...
                    # FIXED LOGIC: Trail the SL to the previous rung
                    if self._current_rung == 1:
                        # Rung 1 Hit -> Move SL to Break-Even (Entry Price)
                        self._current_sl = self._entry_price
                    else:
                        # Rung > 1 Hit -> Move SL to the previous rung's TP level
                        # Subtract 2 to find the multiplier of the PREVIOUS rung
                        prev_ladder_mult = p["ladder_start_r"] + ((self._current_rung - 2) * 2.0)
                        self._current_sl = self._entry_price + prev_ladder_mult * self._R
                    
                    # Next TP increases by exactly 2R
                    ladder_mult = p["ladder_start_r"] + (self._current_rung * 2.0)
        ...
        # First TP (Approximate, recalculated when position opens)
        initial_r = stop_dist
        ...
        # Store pending SL to use if engine doesn't map it immediately
